    ...
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                Grid::make(4)
                    ->schema([
                        TextInput::make('branch.name')->label('Branch')->disabled(),
                        TextInput::make('user.name')->label('User')->disabled(),
                        TextInput::make('event')->disabled(),
                        DateTimePicker::make('created_at')->label('Time')->disabled(),
                    ]),
                Grid::make(2)
                    ->schema([
                        TextInput::make('auditable_type')->label('Record Type')->disabled(),
