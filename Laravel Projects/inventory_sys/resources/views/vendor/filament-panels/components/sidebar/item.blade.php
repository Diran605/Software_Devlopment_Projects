...
<li
    @if ($childItems)
        x-data="{ isExpanded: @js($activeChildItems || $active) }"
    @endif
    {{
        $attributes->class([
            'fi-sidebar-item',
...
>
    ...
    >
        ...
        @if ($childItems)
            <x-filament::icon
                ...
                class="fi-sidebar-item-collapse-icon h-4 w-4 text-gray-400 transition duration-200 ms-auto"
                x-show="$store.sidebar.isOpen"
    ...
    @if ($childItems)
        <ul
            x-show="$store.sidebar.isOpen ? isExpanded : false"
